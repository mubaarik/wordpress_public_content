...
function awpcp_extra_fields_module_installer() {
    return new AWPCP_Extra_Fields_Module_Installer( awpcp_database_helper() );
}

class AWPCP_Extra_Fields_Module_Installer {
    ...
    protected function is_new_installation() {
        return awpcp_table_exists( AWPCP_TABLE_EXTRA_FIELDS ) ? false : true;
    }
    ...
    protected function install_module( $module ) {
        ...
        update_option( 'awpcp-extra-fields-db-version', $module->version );
    }
    ...
    private function upgrade_to_3_0_1($version) {
        ...
        $alter = 'ALTER TABLE ' . AWPCP_TABLE_ADS . ' CHANGE `%s` `%s` %s';
        ...
        foreach ( $fields as $field ) {
            ...
            $wpdb->query( $wpdb->prepare( $alter, $field->field_name, $field->field_name, $change ) );
        }
    }
